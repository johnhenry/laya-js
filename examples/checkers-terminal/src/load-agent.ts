/**
 * Load a `@johnhenry/laya` agent for the Checkers policy (Node/Bun).
 * Loads offline from the local Hugging Face cache by default. Same shape
 * as snake-terminal's/flappy-terminal's `load-agent.ts`.
 */
...
export async function loadCheckersAgent(
  model: string,
  {
    backend = "auto",
    dtype = "f16",
