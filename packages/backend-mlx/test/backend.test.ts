  ...
  describe("backend-mlx", () => {
    ...
    it("supports f32/f16/bf16/i32/bool and round-trips each", async () => {
      ...
      for (const d of ["f32", "f16", "bf16", "i32", "bool"] as const) {
        ...
        const t = be.fromHost(host(d, [3], vals));
    ...
    it("computes in bf16 and f16 and keeps the dtype", async () => {
      ...
      for (const d of ["bf16", "f16"] as const) {
        const x = be.cast(f32([2, 2], [1, 2, 3, 4]), d);
    ...
    it("mlx-c errors become exceptions and the backend stays usable", async () => {
      const be = get();
      assert.throws(() => be.add(f32([2], [1, 2]), f32([3], [1, 2, 3])), /backend-mlx add:.*broadcast/i);
      assert.throws(() => be.reshape(f32([2], [1, 2]), [3]), /backend-mlx reshape/);
      assert.deepEqual([...toF32(await be.read(be.add(f32([1], [1]), f32([1], [2]))))], [3]);
    });

    it("dispose is idempotent, use-after-dispose throws, scope frees intermediates", () => {
      const be = get();
      ...
      assert.throws(() => be.scope(() => { be.exp(f32([1], [1])); throw new Error("boom"); }), /boom/);
      assert.equal(be.liveTensors(), before);
    });

    it("scope/dispose keep MLX memory flat across iterations", () => {
      const be = get();
      const w = be.cast(be.fromHost(host("f32", [256, 256], new Array(256 * 256).fill(0.01))), "f16");
    ...
    it("sdpa accepts a [B,1,1,L] bool padding mask", async () => {
      ...
      const q = be.fromHost(host("f32", [B, H, L, D], Array.from({ length: B * H * L * D }, (_, i) => Math.sin(i))));
      const v = be.fromHost(host("f32", [B, H, L, D], Array.from({ length: B * H * L * D }, (_, i) => i % 7)));
      const mask = be.fromHost(host("bool", [B, 1, 1, L], [1, 1, 0]));
    ...
    it("compile matches eager, supports tensor/array/object outputs and retraces per shape", async () => {
      const be = get();
      let traces = 0;
      const fn = (x: ReturnType<typeof f32>, y: ReturnType<typeof f32>) => {
        traces++;
        return { s: be.add(be.gelu(x), y), p: be.mul(x, y) };
      };
      const cf = be.compile(fn);
      const x = f32([3], [-1, 0, 2]), y = f32([3], [1, 2, 3]);
      const eager = fn(x, y);
      const r = cf(x, y);
    ...
    it("cpu device computes the same results", async () => {
      ...
      const x = cpu.fromHost(host("f32", [2, 2], [1, 2, 3, 4]));
