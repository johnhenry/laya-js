// Derived from Laya (Apache-2.0); see NOTICE. Ported from laya-mlx presets.py.
/** Ready-to-use question presets for common production decision workflows (exact laya-mlx text). */
...
/** Default `emailQuestions` routing categories. */
export const EMAIL_CATEGORIES: Readonly<Record<string, string>> = Object.freeze({
...
export function emailQuestions(categories?: Record<string, string> | null): Questions {
  return {
    category: {
      type: "choice",
      instructions: "Which team should handle the email in `body`?",
      criteria: { ...(categories && Object.keys(categories).length ? categories : EMAIL_CATEGORIES) },
    },
    is_spam: {
      type: "noul",
