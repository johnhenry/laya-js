/**
 * GFLOP/s of quantizedLinear (q8 symmetric / q4 affine, groups of 64, f32
 * accumulation) vs the fp16 Linear, per Laya Linear shape and M = B·L, with
 * the default kernel choice for each (fp16: skinny for M ≤ 64, else
 * subgroup-matrix tiles when available, else direct; quantized:
 * `QUANT_GEMM_DEFAULT`). f16 activations. Interleaved per (shape, M): ROUNDS
 * rounds of ≈WORK flop each, best round; COOL seconds idle between M values
 * (the M2 is fanless).
 *
 *   [MODEL=english|multilingual|both] [MS=1,3,8,16,33,64,93,128,256,1024] [COOL=2] [ROUNDS=5] [WORK=2e10] \
 *   [SRC=<dir with another backend-webgpu src/, e.g. main's, for before/after>] \
...
import type { WebGpuTensor } from "../src/index.ts";

const env = process.env;
const { createWebGpuBackend } = (await import(env.SRC ? `${env.SRC}/index.ts` : "../src/index.ts")) as typeof import("../src/index.ts");
...
const Ms = (env.MS ?? "1,3,8,16,33,64,93,128,256,1024").split(",").map(Number);
